...
void identify_identical_snowflakes(snowflake_node *snowflakes[]) {
        ...
        while (node1 != NULL) {
            ...
            while (node2 != NULL) {
                if (are_identical(node1->snowflake, node2->snowflake)) {
                    printf("Найдены одинаковые снежинки.\n");
                    return;
                }
                node2 = node2->next;
            }
    ...
    printf("Одинаковые снежинки не найдены.\n");
}
...
int main(void) {
    ...
    identify_identical_snowflakes(snowflakes);
    return 0;
}
